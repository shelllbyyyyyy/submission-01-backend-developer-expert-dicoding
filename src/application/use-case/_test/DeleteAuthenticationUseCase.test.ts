...
describe('DeleteAuthenticationUseCase', () => {
  ...
  it('should throw error if use case payload not contain refresh token', async () => {
    ...
    const deleteAuthenticationUseCase = new DeleteAuthenticationUseCase({} as unknown as AuthenticationRepository);

    await expect(deleteAuthenticationUseCase.execute(useCasePayload)).rejects.toThrow(DELETE_AUTHENTICATION_USE_CASE.NOT_CONTAIN_REFRESH_TOKEN);
  });
  ...
  it('should throw error if refresh token not string', async () => {
    ...
    await expect(deleteAuthenticationUseCase.execute(useCasePayload)).rejects.toThrow(DELETE_AUTHENTICATION_USE_CASE.PAYLOAD_NOT_MEET_DATA_TYPE_SPECIFICATION);
  });
